...
@Component({
  selector: 'materials-scanner',
  templateUrl: 'materials-scanner.html',
  providers: [BarcodeScanner]
})

export class MaterialsScanner extends BaseComponent {
  ...
  constructor(
    ...
    this.selectedLocation = params.get('selectedLocation');
  ...
  validateQuantity(data): boolean {
    ...
    if (data) {
      if (data <= 0) {
        ...
        this.setValidationMessage();      //  for Serial # and Asset Tag
      }
      else {
        this.hasValidQuantity = true;
        this.validateFormMessage = "";
  ...
  validateForm(data): boolean {
    ...
    if (this.pickedLine.LotTracked.toLowerCase() == "yes") {
      if (!this.poLinePost.ItemLot) {
        ...
        if (data.receivedQty && data.receivedQty != lotCount) {
          message += '<li>Asset Tag - incorrect count';
          this.validateFormMessage += (this.validateFormMessage == "Please provide") ? "  correct Asset Tag count" : ", correct Asset Tag count";
        }
      }
    }
