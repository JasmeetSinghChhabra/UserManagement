...
import com.accuv.callpushmonitor.AccuvCallConnectionManager;
...
public class FCMService extends FirebaseMessagingService implements PushConstants {
  ...
  public void onMessageReceived(RemoteMessage message) {
    ...
    //ACCUV UPDATE, WITHOUT THIS WE CAN'T GET THE "EXTRAS" DATA SO THAT "CALLS" ARE INSTANTIATED RIGHT
    // WHENT HE APP IS KILLED
    ...
    if (extras != null && isAvailableSender(from)) {
      ...
      for (String bk:extras.keySet()) {
         Log.i(LOG_TAG, " key = " + bk + " value = " + extras.get(bk));
        }
      ...
      // if we are in the foreground and forceShow is `true`, force show the notification if the data has at least a message or title
        ...
        extras.putBoolean(FOREGROUND, true);
        extras.putBoolean(COLDSTART, false);
        if(extras.containsKey("android_channel_id") && "video_call".equalsIgnoreCase(extras.getString("android_channel_id") ) ){
          AccuvCallConnectionManager.setContext(getApplicationContext());
          AccuvCallConnectionManager.onMessageReceived(message, extras);
        }else
